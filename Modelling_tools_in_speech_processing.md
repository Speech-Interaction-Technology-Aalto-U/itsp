# Modelling tools in speech processing

1.  [Source and perceptual modelling](Modelling/Source_modelling_and_perceptual_modelling.md)
1.  [Linear regression](Modelling/Linear_regression.ipynb)
2.  [Sub-space models](Modelling/Sub-space_models)
3.  [Vector quantization (VQ)](Modelling/Vector_quantization_VQ_)
4.  [Gaussian mixture model (GMM)](Modelling/Gaussian_mixture_model_GMM_)
5.  [Neural networks](Modelling/Neural_networks)
6.  [Non-negative Matrix and Tensor Factorization](Modelling/Non-negative_Matrix_and_Tensor_Factorization)
7.  [Hidden Markov Models](https://en.wikipedia.org/wiki/Hidden_Markov_model) (Wikipedia)

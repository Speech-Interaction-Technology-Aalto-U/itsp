...
ix = find(tre(:,2) == 1);
while ix(end) > wincnt-3, ix(end) = []; end
tre(:,3) = tre(:,2);
tre(ix+1,3) = 1;
tre(ix+2,3) = 1;
...
subplot(313)
plot((1:wincnt)*N/(wincnt*fs),[tre(:,1)+.05 tre(:,2)-.05 tre(:,3)-.15])
hold on
axis([0 N/fs -2.2 2.2])
title('VAD decision')
xlabel('Time (s)')
set(gca,'YTick',[-2 -1.5 -1 1 1.5 2 ],'YTicklabel',{'true negative','false negative','non-speech','speech','false positive','true positive'})
legend('Clean','Noisy','Noisy w/ hangover','Location','East')


ix = find((tre(:,1) == 1) & (tre(:,2) == 1));
plot(ix*N/(wincnt*fs),0*ix + 2.1,'kx')
ix = find((tre(:,1) ~= 1) & (tre(:,2) == 1));
plot(ix*N/(wincnt*fs),0*ix + 1.6,'ko')
ix = find((tre(:,1) ~= 1) & (tre(:,2) ~= 1));
plot(ix*N/(wincnt*fs),0*ix - 1.9,'kx')
ix = find((tre(:,1) == 1) & (tre(:,2) ~= 1));
plot(ix*N/(wincnt*fs),0*ix - 1.4,'ko')

ix = find((tre(:,1) == 1) & (tre(:,3) == 1));
plot(ix*N/(wincnt*fs),0*ix + 1.9,'kx')
ix = find((tre(:,1) ~= 1) & (tre(:,3) == 1));
plot(ix*N/(wincnt*fs),0*ix + 1.4,'ko')
ix = find((tre(:,1) ~= 1) & (tre(:,3) ~= 1));
